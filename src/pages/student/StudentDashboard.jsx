...
import { getLocalCourses, getLocalSettings } from '../../utils/mockDatabase';
...
{/* Main Area */}
      <div className="main-offset" style={{ marginLeft: collapsed ? 0 : 240, minHeight: '100vh', transition: 'margin-left 0.25s ease' }}>
...
function StudentOverview({ userProfile, settings, courses }) {
  ...
  const deadline = useMemo(() => {
    return dl ? (dl.seconds ? new Date(dl.seconds * 1000) : new Date(dl)) : null;
  }, [dl]);
      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 1, background: borderColor }}>
          {[
            ...
            const Icon = item.icon;
            return (
              <div key={item.label} style={{ background: cardBg, padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 5 }}>
                  <Icon style={{ width: 11, height: 11, color: textMuted }} />
                  <span style={{ fontSize: 9, fontWeight: 600, color: textMuted, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{item.label}</span>
                </div>
                <div style={{ fontSize: item.accent ? 16 : 13, fontWeight: 700, color: item.accent ? accentColor : textMain, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {item.val || '—'}
                </div>
              </div>
            );
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
        {[
          ...
          return (
            ...
            >
              <div style={{ width: 36, height: 36, borderRadius: 10, background: item.bg, border: '1px solid ' + item.border, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}>
                <Icon style={{ width: 17, height: 17, color: item.color }} />
              </div>
              <div style={{ fontFamily: item.isText ? "'DM Sans', sans-serif" : "'Playfair Display', serif", fontSize: item.isText ? 16 : 28, fontWeight: 700, color: item.isText ? item.color : textMain, lineHeight: 1 }}>
                {item.val}
              </div>
